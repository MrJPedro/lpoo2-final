...
public class AutomovelDAOsql implements AutomovelDAO {
    ...
    private String selectByPlaca = "SELECT " +
    ...
    private String insertVeiculo = "INSERT INTO " +
            "veiculo (id_marca_fk, id_estado_fk, id_categoria_fk, id_locacao_fk, valor_compra, placa, ano) " +
            "VALUES " +
            "(?, ?, ?, ?, ?, ?, ?)";

    private String insertAutomovel = "INSERT INTO " +
            "automovel (placa_fk, id_modelo_fk) " +
            "VALUES " +
            "(?, ?)";

    private String updateVeiculo = "UPDATE veiculo " +
            "SET " +
            "id_marca_fk = ?, " +
            "id_estado_fk = ?, " +
            "id_categoria_fk = ?, " +
    ...
    private String updateAutomovel = "UPDATE automovel " +
            "SET id_modelo_fk = ? " +
            "WHERE placa_fk = ?";
    ...
    private String delete = "DELETE FROM veiculo " +
            "WHERE placa = ?";

    // Deleta todos os registros na tabela veiculo cuja placa se encontra na tabela
    // automovel,
    // o que, como descrito no comentário acima, garante que todos os registros na
    // tabela automovel
    // também sejam excluídos
    private String deleteAll = "DELETE veiculo " +
            "FROM veiculo " +
            "JOIN automovel " +
            ...
            while (rs.next()) {
                ...
                if (dataLocacaoDate != null) {
                    dataLocacao = LocalDate.ofInstant(
                            new java.util.Date(dataLocacaoDate.getTime()).toInstant(),
    ...
    public void add(Automovel automovel) throws SQLException, IOException, Exception {
        try (
            ...
            if (automovel.getLocacao() != null) {
                stVeiculo.setLong(4, automovel.getLocacao().getIdLocacao());
            } else {
                stVeiculo.setNull(4, java.sql.Types.INTEGER);
            }
